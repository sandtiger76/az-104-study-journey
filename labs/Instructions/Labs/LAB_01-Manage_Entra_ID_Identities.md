...
## 🧹 Azure CLI Commands to create users (Alternative)
...
### Step 2: Invite an External User
```bash
az ad user invite \
  --user-principal-name "<your-email>" \
  --display-name "<your-name>" \
  --invite-redirect-url "https://portal.azure.com" \
  --send-invite-message true \
  --invite-message "Welcome to Azure and our group project"
```

### Step 3: Update External User Properties
```bash
az ad user update \
  --id "<your-email>" \
  --job-title "IT Lab Administrator" \
...
az ad group member add \
  ...
  --member-id "<az104-user1-object-id>"
```

```bash
az ad group member add \
  --group "IT Lab Administrators" \
  --member-id "<external-user-object-id>"
```

### Step 3: Get Object IDs
```bash
az ad user show --id "az104-user1@<yourdomain>.onmicrosoft.com" --query objectId
az ad user show --id "<your-email>" --query objectId
```
