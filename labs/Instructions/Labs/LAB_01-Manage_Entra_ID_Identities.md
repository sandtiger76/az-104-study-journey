...
Replace <yourdomain> with your actual Azure AD domain and <AutoGeneratedSecurePassword> with a secure password.

```bash
az ad user create \
    --display-name az104-user1 \
    --password "<AutoGeneratedSecurePassword>" \
    --user-principal-name "az104-user1@<yourdomain>.onmicrosoft.com"
```

### Step 1b: *PowerShell* command to Update New User 
 Replace <yourdomain> with your actual Azure AD domain.

```bash
Update-MgUser `
  -UserId "az104-user1@<yourdomain>.onmicrosoft.com" `
  -JobTitle "IT Lab Administrator" `
  -Department "IT" `
  -UsageLocation "US"
```

### Step 2: *PowerShell* command to Invite an External User
Replace <yourdomain> with your actual Azure AD domain.
```bash
New-MgInvitation `
  -InvitedUserEmailAddress "externaluser@emaildomain" `
  -InvitedUserDisplayName "Name Surname" `
...
You need the object IDs of the users you're adding as owners or members. \
Replace <youruser> with your username. Replace <guestuser> with the external guest user you created. Replace <yourdomain> with your actual Azure AD domain.
```bash
az ad user show --id <youruser>@<yourdomain>.onmicrosoft.com
az ad user show --id az104-user1@<yourdomain>.onmicrosoft.com
az ad user show --id <guestuser>@<yourdomain>.onmicrosoft.com
```

### Step 3: Add Owner to the Group:
```bash
...
Replace "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" with the id of your external user eg. "youremail#EXT#@<yourdomain>.onmicrosoft.com"
```bash
az ad group member add \
  --group "IT Lab Administrators" \
  --member-id "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
...
## 🧹 Azure CLI Commands to Delete Lab 01 Tasks (Alternative)
...
```
az ad user delete --id az104-user1@y<ourdomain>.onmicrosoft.com
```
